...
두 단어가 같은 구성을 갖는 경우,
또는 한 단어에서 한 문자를 더하거나, 빼거나, 하나의 문자를 다른 문자로 바꾸어 나머지 한 단어와 같은 구성을 갖게 되는 경우에
...
// 각 단어를 알파벳별 빈도수(26칸 배열)로 표현한 다음, 첫 단어의 빈도와 비교해서 차이를 세는 방식
void countLetters(const string& s, int arr[26]) {
...
int main() {
    ...
    for (int i = 1; i < n; i++) {
        ...
        int other[26];
        countLetters(w, other);

        int diff = 0; // 두 단어의 알파벳별 개수 차이의 절댓값을 모두 더한 값
        for (int j = 0; j < 26; j++) {
            diff += abs(cnt[j] - other[j]);
        }

        if (diff == 0 || diff == 1) {
            answer++;
        } else if (diff == 2 && w.size() == first.size()) { // 
            answer++;
        }
    }
